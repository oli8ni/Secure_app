...
st.set_page_config(
    page_title="MutuAlert - Sécurité Civique",
    page_icon="🛡️",
    layout="wide",
    initial_sidebar_state="collapsed"
)

# === MASQUER SIDEBAR ET HEADER ===
st.markdown("""
<style>
    ...
    .stat-lbl  { font-size: 0.75rem; color: #666; text-transform: uppercase; letter-spacing: 2px; margin-top: 0.5rem; }
    ...
    .feature-title { color: #fff; font-weight: bold; font-size: 0.9rem; margin-bottom: 0.3rem; }
    .feature-desc  { color: #777; font-size: 0.8rem; }
...
<div class="hero-section">
    ...
    <div class="hero-tagline">Protection Civique en Temps Réel</div>
    <div class="hero-desc">
        <b>MutuAlert</b> connecte instantanément les citoyens en danger avec les forces de l'ordre.
        Géolocalisation précise, suivi d'itinéraires, réponse optimisée.
        Un clic. Une alerte. Une vie sauvée.
    </div>
</div>
""", unsafe_allow_html=True)
...
with col_s3:
    st.markdown(f'<div class="stat-card"><div class="stat-num stat-blue">4.2</div><div class="stat-lbl">Min Réponse</div></div>', unsafe_allow_html=True)
with col_s4:
...
st.markdown('<div class="emergency-label">🚨 ENVOYER UNE ALERTE D\'URGENCE</div>', unsafe_allow_html=True)

_, center, _ = st.columns([1, 1, 1])
with center:
    if st.button("ENVOYER\nALERTE", key="main_alert_btn", type="primary"):
...
with c1:
    ...
    <div class="feature-box">
        ...
        <div class="feature-title">Géolocalisation Auto</div>
        <div class="feature-desc">Votre position GPS est capturée automatiquement avec une précision métrique</div>
    </div>
    """, unsafe_allow_html=True)
with c2:
    st.markdown("""
    <div class="feature-box">
        <div class="feature-icon">⚡</div>
        <div class="feature-title">Alerte Instantanée</div>
...
with c3:
    ...
    <div class="feature-box">
        ...
        <div class="feature-title">Suivi Temps Réel</div>
        <div class="feature-desc">Les forces de l'ordre visualisent votre position avec itinéraire optimisé</div>
    </div>
    """, unsafe_allow_html=True)

# === ACCÈS POLICE (discret) ===
...
with ft3:
    if st.button("🔒", key="secure_trigger", help="Accès réservé"):
...
if st.session_state.show_pin_input:
    with st.container():
        st.markdown("""
        <div style='max-width:400px; margin:0 auto 2rem auto; background:#0a0a14;
                    border:1px solid #1a1a3a; border-radius:12px; padding:1.5rem; text-align:center;'>
        """, unsafe_allow_html=True)
        st.markdown("<h4 style='color:#4B8BFF; margin-top:0;'>🔐 Accès Forces de l'Ordre</h4>", unsafe_allow_html=True)
        st.markdown("<p style='color:#666; font-size:0.85rem;'>Code d'accès requis</p>", unsafe_allow_html=True)

        pin_input = st.text_input("", type="password", placeholder="PIN", key="pin_field", label_visibility="collapsed")

        # ✅ PIN lu depuis l'environnement — jamais en dur dans le code
        POLICE_PIN = os.environ.get("POLICE_PIN", "POLICE2026")
...
st.markdown("""
<div class="footer-co">
    <b>MutuAlert</b> &copy; 2026 | Powered by <a href="https://www.coitechs.com" target="_blank">C&amp;O Itech Solution</a> | Tous droits réservés<br>
    <span style="color:#333;">Sécurité civique en temps réel | POESAM 2026</span>
</div>
""", unsafe_allow_html=True)
