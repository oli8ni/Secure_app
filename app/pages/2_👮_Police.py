...
from modules.database import init_db, get_active_alerts, get_all_alerts, update_alert_status, add_route_data
from modules.auth import authenticate_user, logout, login_required, get_current_user
from modules.geo import validate_coordinates, generate_route_points
from modules.alerts import get_alert_color, get_alert_label, create_blinking_marker_js, format_time_ago
from streamlit_folium import st_folium

init_db()

# Custom CSS for police interface
st.markdown("""
<style>
.police-header {
...
with col_map:
    ...
    if len(df_alerts) > 0:
        ...
        center_lat, center_lon = 48.8566, 2.3522  # Default Paris
    ...
    for idx, alert in df_alerts.iterrows():
        ...
        if status == 'active':
            # Active alerts: pulsing circle + marker
            folium.CircleMarker(
                ...
                fill_color=color,
                fill_opacity=0.3,
                popup=f"Zone d'alerte #{alert['id']}"
            ).add_to(m)
            
            # Main marker
            icon_color = 'red' if status == 'active' else 'orange' if status == 'in_progress' else 'green'
            marker = folium.Marker(
                location=[alert['latitude'], alert['longitude']],
                icon=folium.Icon(color=icon_color, icon='exclamation', prefix='fa'),
                popup=folium.Popup(
                    f"""
                    <div style="font-family:sans-serif; min-width:200px;">
                        <h4 style="color:{color}; margin:0;">🚨 {label} #{alert['id']}</h4>
                        <p><b>Status:</b> <span style="color:{color};">{status.upper()}</span></p>
                        <p><b>Heure:</b> {alert['created_at']}</p>
                        <p><b>Description:</b> {alert['description'] or 'Non spécifié'}</p>
                        <p><b>Tél:</b> {alert['phone'] or 'Anonyme'}</p>
                    </div>
                    """,
                    max_width=300
                ),
            ...
            # Route display for in-progress
            folium.Marker(
                location=[alert['latitude'], alert['longitude']],
                icon=folium.Icon(color='orange', icon='car', prefix='fa'),
                popup=f"Intervention en cours #{alert['id']}"
            ).add_to(m)
            ...
            folium.Marker(
                ...
                popup=f"Résolu #{alert['id']}"
            ).add_to(m)
    
    # Draw routes between HQ and active alerts
    for idx, alert in df_alerts[df_alerts['status'] == 'active'].iterrows():
        route_pts = generate_route_points(
            center_lat, center_lon,
...
with st.expander("📜 Journal d'activité (Audit)"):
    ...
    if len(logs) > 0:
        st.dataframe(logs[['timestamp', 'alert_id', 'action', 'performed_by', 'latitude', 'longitude']], 
